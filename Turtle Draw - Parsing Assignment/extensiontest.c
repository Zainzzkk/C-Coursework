...
void test(void)
{
   ...
   tester = 4;
   assert(scancond(&test, &ctest, filename, tester) == false);
   /*test file*/
   tester = 2;
   assert(scancond(&test, &ctest, filename, tester) == false);
   strcpy(filename, "z.ttl");
   assert(scancond(&test, &ctest, filename, tester) == true);
   assert(test.nme == 0);
   /*will have to do next step due to scanf in terminal*/
   tester = 3;
   assert(scancond(&test, &ctest, filename, tester) == true);
   assert(test.nme == 1);
   /*have to enter characters into terminal*/
   assert(nameentry(&test, &ctest) == true);
   assert(ctest.angle > 269.9999);
   assert(ctest.angle < 270.0001);
   assert(ctest.x1 > 99.9999);
   assert(ctest.x1 < 100.0001);
   assert(ctest.y1 > 399.9999);
   assert(ctest.y1 < 400.0001);
