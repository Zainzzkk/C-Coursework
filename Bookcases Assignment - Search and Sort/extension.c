...
/*makes sure 1 colour on on 1 row*/
...
/*function for plotting route book takes*/
void parentboardroute(bookshelf parents[SIZE], int n);
/*ensures in bounds for route*/
bool inboundcheck(bookshelf parent, int y, int x);
/*adds in arrows to new spot*/
void printshelfroutein(bookshelf* parents, int y, int x);
/*adds in arrows from old spot*/
void printshelfrouteout(bookshelf* parents, int y, int x);
/*shows if going up or down*/
void routeupdown(bookshelf* parents, int newy, int oldy);
/*for printing colours*/
bool ifred(bookshelf parents, int j, int i);
bool ifblack(bookshelf parents, int j, int i);
bool ifgreen(bookshelf parents, int j, int i);
bool ifyellow(bookshelf parents, int j, int i);
bool ifblue(bookshelf parents, int j, int i);
bool ifmagenta(bookshelf parents, int j, int i);
bool ifcyan(bookshelf parents, int j, int i);
bool ifwhite(bookshelf parents, int j, int i);
...
bool checkheightwidth(bookshelf bs, int height, int width)
{
   ...
   if (bs.width > MAXWIDTH - 1 || width > MAXWIDTH - 1 ||
       bs.width == 0 || width == 0){
      return false;
      }
...
void parentboardroute(bookshelf parents[SIZE], int n)
{
   ...
   for(i = 0; i < n; i++){
      ...
      newy = parents[i].newy;
      newx = parents[i].newx;
      if (parents[i].shelf[oldy][oldx] == EMPTY){
         parents[i].shelf[oldy][oldx] = HERE;
         printshelfrouteout(&parents[i], oldy, oldx+1);
         }
      if (inboundcheck(parents[i], newy, newx+1) == true){
         parents[i].shelf[newy][newx+1] = LEFT;
         printshelfroutein(&parents[i], newy, newx+1);
         }
      routeupdown(&parents[i], newy, oldy);
      }
}
...
void printshelfrouteout(bookshelf* parents, int y, int x)
{
   ...
   for (i = x; i < parents->width; i++){
      if(parents->shelf[y][i] == EMPTY){
         parents->shelf[y][i] = RIGHT;
         }
      }
}
...
void routeupdown(bookshelf* parents, int newy, int oldy)
{
   ...
   if (newy > oldy){
      for (i = oldy; i <= newy; i++){
         parents->shelf[i][x] = DOWN;
         }
      }
   if (newy < oldy){
      for (i = oldy ; i >= newy; i--){
         parents->shelf[i][x] = UP;
         }
      }
}
...
void printparents(bookshelf parents[SIZE], int n)
{
   ...
   parentboardroute(parents, n);

   for (k = n; k >= 0; k--){
      for (j = 0; j < parents->height; j++){
         for(i = 0; i <= parents->width; i++){
            if (ifblack(parents[k], j, i)==true){
               neillbgcol(white);
               neillfgcol(black);
               }
            if (ifred(parents[k], j, i)==true){
               neillreset();
               neillfgcol(red);
               }
            if (ifgreen(parents[k], j, i)==true){
               neillreset();
               neillfgcol(green);
               }
            if (ifyellow(parents[k], j, i)==true){
               neillreset();
               neillfgcol(yellow);
               }
            if (ifblue(parents[k], j, i)==true){
               neillreset();
               neillfgcol(blue);
               }
            if (ifyellow(parents[k], j, i)==true){
            ...
            if (ifcyan(parents[k], j, i)==true){
               neillreset();
               neillfgcol(cyan);
               }
            if (ifwhite(parents[k], j, i)==true){
               neillreset();
               neillfgcol(white);
               }
...
bool ifred(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == RED ||( (parents.shelf[newy][newx]==RED)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
      }
   return false;
}
...
bool ifblack(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == BLACK ||( (parents.shelf[newy][newx]==BLACK)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
bool ifgreen(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == GREEN ||( (parents.shelf[newy][newx]==GREEN)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
bool ifyellow(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == YELLOW ||( (parents.shelf[newy][newx]==YELLOW)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
bool ifblue(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == BLUE ||( (parents.shelf[newy][newx]==BLUE)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
bool ifmagenta(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == MAGENTA ||( (parents.shelf[newy][newx]==MAGENTA)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
bool ifcyan(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == CYAN ||( (parents.shelf[newy][newx]==CYAN)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
bool ifwhite(bookshelf parents, int j, int i)
{
   ...
   newy = parents.newy;
   newx = parents.newx;
   if (parents.shelf[j][i] == WHITE ||( (parents.shelf[newy][newx]==WHITE)
       && ( (parents.shelf[j][i] == LEFT)||(parents.shelf[j][i] == RIGHT)
       ||(parents.shelf[j][i] == UP)||(parents.shelf[j][i] == DOWN)
       ||(parents.shelf[j][i] == HERE) ))){
      return true;
...
void test(void)
{
   ...
   /*some blanked out for extension as otherwise fails*/
   bookshelf b;
   int i;
   ...
   /*tests printing on final board correctly*/
   parentboardroute(testparents, 2);
   memcpy(&b, &testparents[0], sizeof(bookshelf));
   assert(b.shelf[0][0]==(YELLOW));
   assert(b.shelf[0][1]==(YELLOW));
   assert(b.shelf[0][2]==(YELLOW));
   assert(b.shelf[0][3]==(UP));
   assert(b.shelf[1][0]==(BLUE));
   assert(b.shelf[1][1]==(BLUE));
   assert(b.shelf[1][2]==(BLUE));
   assert(b.shelf[1][3]==(UP));
   assert(b.shelf[2][0]==(HERE));
   assert(b.shelf[2][1]==(RIGHT));
   assert(b.shelf[2][2]==(RIGHT));
   assert(b.shelf[2][3]==(UP));
   ...
   b.shelf[1][1]=EMPTY;
   b.shelf[0][2]=EMPTY;
   memcpy(&testparents[0], &b, sizeof(bookshelf));
   assert(inboundcheck(testparents[0],1,1)==true);
   parentboardroute(testparents, 2);
   memcpy(&b, &testparents[0], sizeof(bookshelf));
   assert(b.shelf[0][1]==(HERE));
   assert(b.shelf[1][0]==(BLUE));
   assert(b.shelf[1][1]==(LEFT));
   printshelfrouteout(&testparents[0], 0, 2);
   memcpy(&b, &testparents[0], sizeof(bookshelf));
   assert(b.shelf[0][2]==(RIGHT));
   printshelfroutein(&testparents[0], 1, 1);
   memcpy(&b, &testparents[0], sizeof(bookshelf));
   assert(b.shelf[1][2]==(LEFT));
   routeupdown(&testparents[0],1,0);
   memcpy(&b, &testparents[0], sizeof(bookshelf));
   assert(b.shelf[0][3]==(DOWN));
   assert(b.shelf[1][3]==(DOWN));
}
